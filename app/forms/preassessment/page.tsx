...
// ==========================================
// MOVED OUTSIDE: Reusable Components
// This prevents the "losing focus" bug!
// ==========================================
const FormInput = ({ 
  label, 
  placeholder, 
...
export default function ClinicalRegistrationFlow() {
  ...
  // Show a quick loader while grabbing the profile data so the form doesn't "jump"
        ...
        <main className="max-w-4xl mx-auto px-6 pt-6 relative z-10">
          ...
          <form className="space-y-1" onSubmit={handleFormSubmit}>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-x-6 gap-y-3">
              ...
              <FormInput label="Address" name="address" value={formData.address} onChange={handleInputChange} placeholder="Type here" />
              ...
              <FormInput label="Programme" name="programme" value={formData.programme} onChange={handleInputChange} placeholder="Doctor/Bachelor/Diploma" />
            </div>

            <SectionHeader title="SECTION B: Staffing Information" />
            <div className="grid grid-cols-1 md:grid-cols-2 gap-x-6 gap-y-3">
              <FormInput label="Number of Clinical Staff" name="clinicalStaff" value={formData.clinicalStaff} onChange={handleInputChange} placeholder="Type here" type="number" />
              <FormInput label="Number of Administrative Staff" name="adminStaff" value={formData.adminStaff} onChange={handleInputChange} placeholder="Type here" type="number" />
            </div>

            <SectionHeader title="SECTION C: Physical Facility" />
            <div className="grid grid-cols-1 md:grid-cols-2 gap-x-6 gap-y-3">
              <FormInput label="Number of Offices" name="offices" value={formData.offices} onChange={handleInputChange} placeholder="23" type="number" />
              <FormInput label="Number of Conveniences / Toilets" name="conveniences" value={formData.conveniences} onChange={handleInputChange} placeholder="5" type="number" />
              <FormInput label="Number of Treatment Rooms" name="treatmentRooms" value={formData.treatmentRooms} onChange={handleInputChange} placeholder="15" type="number" />
              <FormInput label="Other Physical Facility" name="otherFacility" value={formData.otherFacility} onChange={handleInputChange} placeholder="Gym, Gait Training Room, Workshops, etc" />
            </div>

            <SectionHeader title="SECTION D: Equipment Inventory" />
            <div className="flex flex-col gap-3 w-full">
              <div className="flex justify-between items-end ml-1">
                <label className="text-[13px] font-bold text-gray-700">Equipment List</label>
              ...
              {equipmentFields.map((field, index) => (
                <div key={index} className="flex items-center gap-3 w-full animate-in slide-in-from-right-2 duration-300">
                  <div className="relative flex-grow">
                    ...
                    />
                    <button type="button" onClick={() => removeEquipmentField(index)} className="absolute right-3 top-1/2 -translate-y-1/2 p-1 text-gray-300 hover:text-red-500">
                      <X size={18} />
                    </button>
                  </div>
              ...
              <button type="button" onClick={addMoreEquipment} className="w-full flex items-center justify-center gap-2 border-2 border-dashed border-[#CDE1B4] hover:border-[#5D9C0E] text-[#5D9C0E] py-3 rounded-xl text-xs font-bold transition-all bg-gray-50/30 hover:bg-[#f8fcf5]">
                <Plus size={16} /> Add Another Equipment Field
              </button>
            </div>
