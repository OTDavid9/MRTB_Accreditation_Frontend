...
export default function StepOne({ data, updateData, onNext }: StepOneProps) {
  const { lecturers, supportStaff } = data;

  const addLecturer = () => updateData({ ...data, lecturers: [...lecturers, { id: Date.now(), name: '', gender: '', dateAppt: '', natureAppt: '', designation: '', license: '', specialization: '', qualifications: [{ id: Date.now() + 1, title: '', date: '' }], cpds: [{ id: Date.now() + 2, title: '' }], papers: [{ id: Date.now() + 3, title: '' }] }] });
  const removeLecturer = (id: number) => updateData({ ...data, lecturers: lecturers.filter((p: any) => p.id !== id) });
  const updateLecturer = (id: number, field: string, value: any) => updateData({ ...data, lecturers: lecturers.map((p: any) => p.id === id ? { ...p, [field]: value } : p) });
  
  const addListReq = (pId: number, listField: string) => updateData({ ...data, lecturers: lecturers.map((p: any) => p.id === pId ? { ...p, [listField]: [...p[listField], { id: Date.now(), title: '', date: '' }] } : p) });
  const removeListReq = (pId: number, rId: number, listField: string) => updateData({ ...data, lecturers: lecturers.map((p: any) => p.id === pId ? { ...p, [listField]: p[listField].filter((q: any) => q.id !== rId) } : p) });
  const updateListReq = (pId: number, rId: number, listField: string, field: string, val: string) => updateData({ ...data, lecturers: lecturers.map((p: any) => p.id === pId ? { ...p, [listField]: p[listField].map((q: any) => q.id === rId ? { ...q, [field]: val } : q) } : p) });
    ...
    <div className="flex flex-col items-center animate-in fade-in slide-in-from-bottom-4 duration-500">
      <div className="text-center mb-10 max-w-2xl relative z-10">
        ...
        <p className="text-[14px] text-gray-600 leading-relaxed">
          The staff accreditation form should be completed by each Lecturer prior to the screening exercise.
        </p>
      </div>
      ...
      <div className="w-full relative flex flex-col items-center mb-12">
        <div className="bg-[#F4F9F2] text-[#5D9C0E] px-8 py-2.5 rounded-md text-[15px] font-bold inline-block mb-6 z-10 tracking-wider">
          i. Accreditation of Lecturers
        </div>

        <div className="w-full flex flex-col gap-6 relative z-10">
          {lecturers.map((lecturer: any, index: number) => (
            <div key={lecturer.id} className="w-full bg-white p-6 md:p-8 rounded-2xl shadow-[0px_4px_20px_rgba(0,0,0,0.03)] border border-gray-50">
              {lecturers.length > 1 && (
                <div className="flex justify-between items-center mb-6 pb-4 border-b border-gray-100">
                  <h3 className="font-bold text-gray-700">Lecturer #{index + 1}</h3>
                  <button onClick={() => removeLecturer(lecturer.id)} className="text-red-400 hover:text-red-600 flex items-center gap-1 text-xs font-bold transition-colors">
                    <X size={14} /> Remove
                  </button>
                </div>
              )}
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-8">
                <InputField label="Name" placeholder="Type here" value={lecturer.name} onChange={(v: string) => updateLecturer(lecturer.id, 'name', v)} />
                <InputField label="Gender" type="select" placeholder="Select Gender" options={["Male", "Female"]} value={lecturer.gender} onChange={(v: string) => updateLecturer(lecturer.id, 'gender', v)} />
                <InputField label="Date of first appointment" type="date" value={lecturer.dateAppt} onChange={(v: string) => updateLecturer(lecturer.id, 'dateAppt', v)} />
                <InputField label="Nature of Appointment" type="select" placeholder="Select Nature" options={["Full-Time", "Part-Time", "Contract"]} value={lecturer.natureAppt} onChange={(v: string) => updateLecturer(lecturer.id, 'natureAppt', v)} />
                <InputField label="Designation" placeholder="Type here" value={lecturer.designation} onChange={(v: string) => updateLecturer(lecturer.id, 'designation', v)} />
                <InputField label="MRTB License No" placeholder="Type here" value={lecturer.license} onChange={(v: string) => updateLecturer(lecturer.id, 'license', v)} />
                <InputField label="Areas of Specialization" placeholder="e.g. Speech Sound Disorders" value={lecturer.specialization} onChange={(v: string) => updateLecturer(lecturer.id, 'specialization', v)} />
              </div>
              ...
              <div className="mb-6 p-5 bg-[#FAFAFA] rounded-xl border border-gray-100">
                ...
                <div className="space-y-3">
                  {lecturer.qualifications.map((qual: any) => (
                    <div key={qual.id} className="flex flex-col sm:flex-row items-start sm:items-center gap-3">
                      <div className="flex-1 w-full">
                        <input type="text" placeholder="Degree / Certification Name" value={qual.title} onChange={(e) => updateListReq(lecturer.id, qual.id, 'qualifications', 'title', e.target.value)} className="w-full border border-[#BDE0A6] rounded-md p-2.5 outline-none focus:ring-2 focus:ring-[#5D9C0E]/50 text-[13px] bg-white transition-all" />
                      </div>
                      <div className="w-full sm:w-[160px] shrink-0">
                        <input type="date" value={qual.date} onChange={(e) => updateListReq(lecturer.id, qual.id, 'qualifications', 'date', e.target.value)} className="w-full border border-[#BDE0A6] rounded-md p-2.5 outline-none focus:ring-2 focus:ring-[#5D9C0E]/50 text-[13px] bg-white transition-all" />
                      </div>
                      {lecturer.qualifications.length > 1 && (
                        <button onClick={() => removeListReq(lecturer.id, qual.id, 'qualifications')} className="p-2.5 text-gray-400 hover:text-red-500 shrink-0"><Trash2 size={16} /></button>
                      )}
                    </div>
                  ))}
                </div>
                <button onClick={() => addListReq(lecturer.id, 'qualifications')} className="mt-3 text-[13px] text-[#78C822] font-medium hover:underline">+ Add another qualification</button>
              </div>

              {/* Dynamic CPDs List */}
              <div className="mb-6 p-5 bg-[#FAFAFA] rounded-xl border border-gray-100">
                <label className="text-[14px] text-gray-800 font-medium mb-3 block">CPD Programmes Attended (Last 12 Months)</label>
                <div className="space-y-3">
                  {lecturer.cpds.map((cpd: any) => (
                    <div key={cpd.id} className="flex items-center gap-3">
                      <input type="text" placeholder="Title of CPD program" value={cpd.title} onChange={(e) => updateListReq(lecturer.id, cpd.id, 'cpds', 'title', e.target.value)} className="flex-1 border border-[#BDE0A6] rounded-md p-2.5 outline-none focus:ring-2 focus:ring-[#5D9C0E]/50 text-[13px] bg-white transition-all" />
                      {lecturer.cpds.length > 1 && (
                        <button onClick={() => removeListReq(lecturer.id, cpd.id, 'cpds')} className="p-2.5 text-gray-400 hover:text-red-500 shrink-0"><Trash2 size={16} /></button>
                      )}
                    </div>
                  ))}
                </div>
                <button onClick={() => addListReq(lecturer.id, 'cpds')} className="mt-3 text-[13px] text-[#78C822] font-medium hover:underline">+ Add another CPD</button>
              </div>

              {/* Dynamic Papers List */}
              <div className="p-5 bg-[#FAFAFA] rounded-xl border border-gray-100">
                <label className="text-[14px] text-gray-800 font-medium mb-3 block">Papers Presented (Last 12 Months)</label>
                <div className="space-y-3">
                  {lecturer.papers.map((paper: any) => (
                    <div key={paper.id} className="flex items-center gap-3">
                      <input type="text" placeholder="Title of Paper Presented" value={paper.title} onChange={(e) => updateListReq(lecturer.id, paper.id, 'papers', 'title', e.target.value)} className="flex-1 border border-[#BDE0A6] rounded-md p-2.5 outline-none focus:ring-2 focus:ring-[#5D9C0E]/50 text-[13px] bg-white transition-all" />
                      {lecturer.papers.length > 1 && (
                        <button onClick={() => removeListReq(lecturer.id, paper.id, 'papers')} className="p-2.5 text-gray-400 hover:text-red-500 shrink-0"><Trash2 size={16} /></button>
                      )}
                    </div>
                  ))}
                </div>
                <button onClick={() => addListReq(lecturer.id, 'papers')} className="mt-3 text-[13px] text-[#78C822] font-medium hover:underline">+ Add another Paper</button>
              </div>
        ...
        <div className="w-full flex justify-end mt-4 z-10">
          <button onClick={addLecturer} className="border border-[#78C822] text-[#78C822] px-4 py-2 rounded-md text-[14px] font-medium hover:bg-[#F4F9F2] transition-colors">+ Add More Lecturers</button>
        </div>
      </div>
      ...
      <div className="w-full relative flex flex-col items-center mb-12">
        ...
        <p className="text-[12px] text-gray-400 mb-6 z-10">Screening for Support Staff should be done after that of the Lecturers.</p>
        ...
        <div className="w-full flex flex-col gap-6 relative z-10">
          {supportStaff.map((staff: any, index: number) => (
            <div key={staff.id} className="w-full bg-white p-6 md:p-8 rounded-2xl shadow-[0px_4px_20px_rgba(0,0,0,0.03)] border border-gray-50">
              ...
              <div className="mt-2">
                <label className="text-[14px] text-gray-800 font-medium mb-1 block">Evidence of cont. training</label>
