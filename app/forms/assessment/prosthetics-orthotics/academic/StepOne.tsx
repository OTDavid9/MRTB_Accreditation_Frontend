...
export default function StepOne({ data, updateData, onNext }: StepOneProps) {
  ...
  const addLecturer = () => updateData({ ...data, lecturers: [...lecturers, { id: Date.now(), name: '', gender: '', qualification: '', designation: '', license: '', cpds: [{ id: Date.now() + 1, title: '' }], pgCerts: [{ id: Date.now() + 2, title: '' }] }] });
  ...
  const addListReq = (pId: number, listField: string) => updateData({ ...data, lecturers: lecturers.map((p: any) => p.id === pId ? { ...p, [listField]: [...p[listField], { id: Date.now(), title: '' }] } : p) });
    ...
    <div className="flex flex-col items-center animate-in fade-in slide-in-from-bottom-4 duration-500">
      <div className="text-center mb-10 max-w-2xl relative z-10">
        ...
        <p className="text-[14px] text-gray-600 leading-relaxed">
          The Academic form should be completed by each Prosthetics & Orthotics Lecturer prior to the screening exercise.
        </p>
      </div>
      ...
      <div className="w-full relative flex flex-col items-center mb-12">
        <div className="bg-[#F4F9F2] text-[#5D9C0E] px-8 py-2.5 rounded-md text-[15px] font-bold inline-block mb-6 z-10 tracking-wider">
          i. Medical Rehabilitation Therapists (Lecturers)
        </div>
          ...
          {lecturers.map((lecturer: any, index: number) => (
            <div key={lecturer.id} className="w-full bg-white p-6 md:p-8 rounded-2xl shadow-[0px_4px_20px_rgba(0,0,0,0.03)] border border-gray-50">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-8">
                <InputField label="Names" placeholder="Type here" value={lecturer.name} onChange={(v: string) => updateLecturer(lecturer.id, 'name', v)} />
                <InputField label="Sex (Gender)" type="select" placeholder="Select Gender" options={["Male", "Female"]} value={lecturer.gender} onChange={(v: string) => updateLecturer(lecturer.id, 'gender', v)} />
                <InputField label="Qualification" placeholder="Type here" value={lecturer.qualification} onChange={(v: string) => updateLecturer(lecturer.id, 'qualification', v)} />
              ...
              <div className="mb-6 p-5 bg-[#FAFAFA] rounded-xl border border-gray-100">
                <label className="text-[14px] text-gray-800 font-medium mb-3 block">PG Certificate(s) obtained after 1st Degree</label>
                <div className="space-y-3">
                  {lecturer.pgCerts.map((cert: any) => (
                    <div key={cert.id} className="flex items-center gap-3">
                      <input type="text" placeholder="Certificate Name" value={cert.title} onChange={(e) => updateListReq(lecturer.id, cert.id, 'pgCerts', 'title', e.target.value)} className="flex-1 border border-[#BDE0A6] rounded-md p-2.5 outline-none focus:ring-2 focus:ring-[#5D9C0E]/50 text-[13px] bg-white transition-all" />
                      {lecturer.pgCerts.length > 1 && (
                        <button onClick={() => removeListReq(lecturer.id, cert.id, 'pgCerts')} className="p-2.5 text-gray-400 hover:text-red-500 shrink-0"><Trash2 size={16} /></button>
                      )}
                    </div>
                  ))}
                </div>
                <button onClick={() => addListReq(lecturer.id, 'pgCerts')} className="mt-3 text-[13px] text-[#78C822] font-medium hover:underline">+ Add another PG Certificate</button>
              </div>
