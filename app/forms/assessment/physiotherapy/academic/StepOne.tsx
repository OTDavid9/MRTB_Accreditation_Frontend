...
export default function StepOne({ data, updateData, onNext }: StepOneProps) {
  ...
  // LECTURER LOGIC
  const addLecturer = () => updateData({ ...data, lecturers: [...lecturers, { id: Date.now(), name: '', gender: '', dateAppt: '', natureAppt: '', designation: '', license: '', qualifications: [{ id: Date.now() + 1, title: '', date: '' }], cpds: [{ id: Date.now() + 2, title: '' }] }] });
  const removeLecturer = (id: number) => updateData({ ...data, lecturers: lecturers.filter((l: any) => l.id !== id) });
  const updateLecturer = (id: number, field: string, value: any) => updateData({ ...data, lecturers: lecturers.map((l: any) => l.id === id ? { ...l, [field]: value } : l) });
  
  const addLecQual = (lId: number) => updateData({ ...data, lecturers: lecturers.map((l: any) => l.id === lId ? { ...l, qualifications: [...l.qualifications, { id: Date.now(), title: '', date: '' }] } : l) });
  const removeLecQual = (lId: number, qId: number) => updateData({ ...data, lecturers: lecturers.map((l: any) => l.id === lId ? { ...l, qualifications: l.qualifications.filter((q: any) => q.id !== qId) } : l) });
  const updateLecQual = (lId: number, qId: number, field: string, val: string) => updateData({ ...data, lecturers: lecturers.map((l: any) => l.id === lId ? { ...l, qualifications: l.qualifications.map((q: any) => q.id === qId ? { ...q, [field]: val } : q) } : l) });

  const addLecCpd = (lId: number) => updateData({ ...data, lecturers: lecturers.map((l: any) => l.id === lId ? { ...l, cpds: [...l.cpds, { id: Date.now(), title: '' }] } : l) });
  const removeLecCpd = (lId: number, cId: number) => updateData({ ...data, lecturers: lecturers.map((l: any) => l.id === lId ? { ...l, cpds: l.cpds.filter((c: any) => c.id !== cId) } : l) });
  const updateLecCpd = (lId: number, cId: number, val: string) => updateData({ ...data, lecturers: lecturers.map((l: any) => l.id === lId ? { ...l, cpds: l.cpds.map((c: any) => c.id === cId ? { ...c, title: val } : c) } : l) });
  ...
  return (
    <div className="flex flex-col items-center animate-in fade-in slide-in-from-bottom-4 duration-500">
      ...
      <div className="w-full relative flex flex-col items-center mb-12">
        <div className="bg-[#F4F9F2] text-[#5D9C0E] px-8 py-2.5 rounded-md text-[15px] font-medium inline-block mb-6 z-10">
          Lecturers Accreditation
        </div>
          ...
          {lecturers.map((lecturer: any, index: number) => (
            <div key={lecturer.id} className="w-full bg-white p-6 md:p-8 rounded-2xl shadow-[0px_4px_20px_rgba(0,0,0,0.03)] border border-gray-50">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-8">
                ...
                <InputField label="Nature of appointment" type="select" placeholder="Select Nature" options={["Temporal", "Permanent"]} value={lecturer.natureAppt} onChange={(v: string) => updateLecturer(lecturer.id, 'natureAppt', v)} />
                  ...
                  {lecturer.qualifications.map((qual: any) => (
                    <div key={qual.id} className="flex flex-col sm:flex-row items-start sm:items-center gap-3">
                      <div className="flex-1 w-full">
                        <input type="text" placeholder="Degree / Certification Name" value={qual.title} onChange={(e) => updateLecQual(lecturer.id, qual.id, 'title', e.target.value)} className="w-full border border-[#BDE0A6] rounded-md p-2.5 outline-none focus:ring-2 focus:ring-[#5D9C0E]/50 text-[13px] bg-white transition-all" />
                      </div>
                      <div className="w-full sm:w-[160px] shrink-0">
                        <input type="date" value={qual.date} onChange={(e) => updateLecQual(lecturer.id, qual.id, 'date', e.target.value)} className="w-full border border-[#BDE0A6] rounded-md p-2.5 outline-none focus:ring-2 focus:ring-[#5D9C0E]/50 text-[13px] bg-white transition-all" />
                      </div>
                      {lecturer.qualifications.length > 1 && (
                        <button onClick={() => removeLecQual(lecturer.id, qual.id)} className="p-2.5 text-gray-400 hover:text-red-500 shrink-0"><Trash2 size={16} /></button>
                      )}
                    </div>
                  ))}
                </div>
                <button onClick={() => addLecQual(lecturer.id)} className="mt-3 text-[13px] text-[#78C822] font-medium hover:underline">+ Add another qualification</button>
              </div>

              {/* Dynamic CPDs List */}
              <div className="p-5 bg-[#FAFAFA] rounded-xl border border-gray-100">
                <label className="text-[14px] text-gray-800 font-medium mb-3 block">Paper Presented / CPD Attended (Past 12 months)</label>
                <div className="space-y-3">
                  {lecturer.cpds.map((cpd: any) => (
                    <div key={cpd.id} className="flex items-center gap-3">
                      <input type="text" placeholder="Title of paper or CPD" value={cpd.title} onChange={(e) => updateLecCpd(lecturer.id, cpd.id, e.target.value)} className="flex-1 border border-[#BDE0A6] rounded-md p-2.5 outline-none focus:ring-2 focus:ring-[#5D9C0E]/50 text-[13px] bg-white transition-all" />
                      {lecturer.cpds.length > 1 && (
                        <button onClick={() => removeLecCpd(lecturer.id, cpd.id)} className="p-2.5 text-gray-400 hover:text-red-500 shrink-0"><Trash2 size={16} /></button>
                      )}
                    </div>
                  ))}
                </div>
                <button onClick={() => addLecCpd(lecturer.id)} className="mt-3 text-[13px] text-[#78C822] font-medium hover:underline">+ Add another paper/CPD</button>
              </div>
      ...
      <div className="w-full relative flex flex-col items-center mb-12">
        <div className="bg-[#F4F9F2] px-8 py-2.5 rounded-md text-[15px] font-medium inline-block mb-2 z-10 text-[#5D9C0E]">
          ...
          {supportStaff.map((staff: any, index: number) => (
            <div key={staff.id} className="w-full bg-white p-6 md:p-8 rounded-2xl shadow-[0px_4px_20px_rgba(0,0,0,0.03)] border border-gray-50">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-8">
                ...
                <InputField label="Present Rank/Cadre" placeholder="Type here" value={staff.rank} onChange={(v: string) => updateSupport(staff.id, 'rank', v)} />
              ...
              {/* Dynamic Qualifications for Support Staff */}
              <div className="mb-6 p-5 bg-[#FAFAFA] rounded-xl border border-gray-100">
              ...
              <div className="mt-2">
                ...
                <FileUploadBlock 
                  file={staff.trainingFileName} 
                  onUpload={(name) => updateSupport(staff.id, 'trainingFileName', name)} 
                  onRemove={() => updateSupport(staff.id, 'trainingFileName', '')} 
                />
              </div>
