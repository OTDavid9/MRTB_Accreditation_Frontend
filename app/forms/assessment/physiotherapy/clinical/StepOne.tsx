...
export default function StepOne({ data, updateData, onNext }: StepOneProps) {
  const { physios, supportStaff } = data;

  const addPhysio = () => updateData({ ...data, physios: [...physios, { id: Date.now(), name: '', gender: '', dateAppt: '', designation: '', license: '', specialization: '', qualifications: [{ id: Date.now() + 1, title: '', date: '' }], cpds: [{ id: Date.now() + 2, title: '' }] }] });
  const removePhysio = (id: number) => updateData({ ...data, physios: physios.filter((p: any) => p.id !== id) });
  const updatePhysio = (id: number, field: string, value: any) => updateData({ ...data, physios: physios.map((p: any) => p.id === id ? { ...p, [field]: value } : p) });
  
  const addQual = (pId: number) => updateData({ ...data, physios: physios.map((p: any) => p.id === pId ? { ...p, qualifications: [...p.qualifications, { id: Date.now(), title: '', date: '' }] } : p) });
  const removeQual = (pId: number, qId: number) => updateData({ ...data, physios: physios.map((p: any) => p.id === pId ? { ...p, qualifications: p.qualifications.filter((q: any) => q.id !== qId) } : p) });
  const updateQual = (pId: number, qId: number, field: string, val: string) => updateData({ ...data, physios: physios.map((p: any) => p.id === pId ? { ...p, qualifications: p.qualifications.map((q: any) => q.id === qId ? { ...q, [field]: val } : q) } : p) });

  const addCpd = (pId: number) => updateData({ ...data, physios: physios.map((p: any) => p.id === pId ? { ...p, cpds: [...p.cpds, { id: Date.now(), title: '' }] } : p) });
  const removeCpd = (pId: number, cId: number) => updateData({ ...data, physios: physios.map((p: any) => p.id === pId ? { ...p, cpds: p.cpds.filter((c: any) => c.id !== cId) } : p) });
  const updateCpd = (pId: number, cId: number, val: string) => updateData({ ...data, physios: physios.map((p: any) => p.id === pId ? { ...p, cpds: p.cpds.map((c: any) => c.id === cId ? { ...c, title: val } : c) } : p) });
    ...
    <div className="flex flex-col items-center animate-in fade-in slide-in-from-bottom-4 duration-500">
      <div className="text-center mb-10 max-w-2xl relative z-10">
        ...
        <p className="text-[14px] text-gray-600 leading-relaxed">
          The staff Accreditation form should be completed by each Physiotherapist prior to the screening exercise.
        </p>
      </div>
      ...
      <div className="w-full relative flex flex-col items-center mb-12">
        <div className="bg-[#F4F9F2] text-[#5D9C0E] px-8 py-2.5 rounded-md text-[15px] font-bold inline-block mb-6 z-10 tracking-wider">
          i. Accreditation of Physiotherapists
        </div>

        <div className="w-full flex flex-col gap-6 relative z-10">
          {physios.map((physio: any, index: number) => (
            <div key={physio.id} className="w-full bg-white p-6 md:p-8 rounded-2xl shadow-[0px_4px_20px_rgba(0,0,0,0.03)] border border-gray-50">
              {physios.length > 1 && (
                <div className="flex justify-between items-center mb-6 pb-4 border-b border-gray-100">
                  <h3 className="font-bold text-gray-700">Physiotherapist #{index + 1}</h3>
                  <button onClick={() => removePhysio(physio.id)} className="text-red-400 hover:text-red-600 flex items-center gap-1 text-xs font-bold transition-colors">
                    <X size={14} /> Remove
                  </button>
                </div>
              )}
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-8">
                <InputField label="Name" placeholder="Type here" value={physio.name} onChange={(v: string) => updatePhysio(physio.id, 'name', v)} />
                <InputField label="Gender" type="select" placeholder="Select Gender" options={["Male", "Female"]} value={physio.gender} onChange={(v: string) => updatePhysio(physio.id, 'gender', v)} />
                <InputField label="Date of first appointment" type="date" value={physio.dateAppt} onChange={(v: string) => updatePhysio(physio.id, 'dateAppt', v)} />
                <InputField label="Designation" placeholder="Type here" value={physio.designation} onChange={(v: string) => updatePhysio(physio.id, 'designation', v)} />
                <InputField label="MRTB License No" placeholder="Type here" value={physio.license} onChange={(v: string) => updatePhysio(physio.id, 'license', v)} />
                <InputField label="Areas of Specialization" placeholder="e.g. Paediatrics, Neurology" value={physio.specialization} onChange={(v: string) => updatePhysio(physio.id, 'specialization', v)} />
              </div>
              ...
              <div className="mb-6 p-5 bg-[#FAFAFA] rounded-xl border border-gray-100">
                ...
                <div className="space-y-3">
                  {physio.qualifications.map((qual: any) => (
                    <div key={qual.id} className="flex flex-col sm:flex-row items-start sm:items-center gap-3">
                      <div className="flex-1 w-full">
                        <input type="text" placeholder="Degree / Certification Name" value={qual.title} onChange={(e) => updateQual(physio.id, qual.id, 'title', e.target.value)} className="w-full border border-[#BDE0A6] rounded-md p-2.5 outline-none focus:ring-2 focus:ring-[#5D9C0E]/50 text-[13px] bg-white transition-all" />
                      </div>
                      <div className="w-full sm:w-[160px] shrink-0">
                        <input type="date" value={qual.date} onChange={(e) => updateQual(physio.id, qual.id, 'date', e.target.value)} className="w-full border border-[#BDE0A6] rounded-md p-2.5 outline-none focus:ring-2 focus:ring-[#5D9C0E]/50 text-[13px] bg-white transition-all" />
                      </div>
                      {physio.qualifications.length > 1 && (
                        <button onClick={() => removeQual(physio.id, qual.id)} className="p-2.5 text-gray-400 hover:text-red-500 shrink-0"><Trash2 size={16} /></button>
                      )}
                    </div>
                  ))}
                </div>
                <button onClick={() => addQual(physio.id)} className="mt-3 text-[13px] text-[#78C822] font-medium hover:underline">+ Add another qualification</button>
              </div>

              {/* Dynamic CPDs List */}
              <div className="p-5 bg-[#FAFAFA] rounded-xl border border-gray-100">
                <label className="text-[14px] text-gray-800 font-medium mb-3 block">No. of CPD in the last 12 months (List them)</label>
                <div className="space-y-3">
                  {physio.cpds.map((cpd: any) => (
                    <div key={cpd.id} className="flex items-center gap-3">
                      <input type="text" placeholder="Title of CPD program" value={cpd.title} onChange={(e) => updateCpd(physio.id, cpd.id, e.target.value)} className="flex-1 border border-[#BDE0A6] rounded-md p-2.5 outline-none focus:ring-2 focus:ring-[#5D9C0E]/50 text-[13px] bg-white transition-all" />
                      {physio.cpds.length > 1 && (
                        <button onClick={() => removeCpd(physio.id, cpd.id)} className="p-2.5 text-gray-400 hover:text-red-500 shrink-0"><Trash2 size={16} /></button>
                      )}
                    </div>
                  ))}
                </div>
                <button onClick={() => addCpd(physio.id)} className="mt-3 text-[13px] text-[#78C822] font-medium hover:underline">+ Add another CPD</button>
              </div>
        ...
        <div className="w-full flex justify-end mt-4 z-10">
          <button onClick={addPhysio} className="border border-[#78C822] text-[#78C822] px-4 py-2 rounded-md text-[14px] font-medium hover:bg-[#F4F9F2] transition-colors">+ Add More Physiotherapists</button>
        </div>
      </div>
      ...
      <div className="w-full relative flex flex-col items-center mb-12">
        ...
        <p className="text-[12px] text-gray-400 mb-6 z-10">Screening for Support Staff should be done after that of the Physiotherapists.</p>
