...
export default function StepOne({ data, updateData, onNext }: StepOneProps) {
  ...
  const addLecturer = () => updateData({ ...data, lecturers: [...lecturers, { id: Date.now(), name: '', gender: '', dateAppt: '', natureAppt: '', designation: '', license: '', specialization: '', qualifications: [{ id: Date.now() + 1, title: '', date: '' }], journals: [{ id: Date.now() + 2, title: '' }] }] });
          ...
          {lecturers.map((lecturer: any, index: number) => (
            <div key={lecturer.id} className="w-full bg-white p-6 md:p-8 rounded-2xl shadow-[0px_4px_20px_rgba(0,0,0,0.03)] border border-gray-50">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-8">
                ...
                <InputField label="Nature of Appointment" type="select" placeholder="Select Nature" options={["Temporary", "Permanent"]} value={lecturer.natureAppt} onChange={(v: string) => updateLecturer(lecturer.id, 'natureAppt', v)} />
                ...
                <InputField label="Areas of Specialization" placeholder="e.g. Ergonomics, Mental Health" value={lecturer.specialization} onChange={(v: string) => updateLecturer(lecturer.id, 'specialization', v)} />
              </div>
              ...
              {/* Dynamic CPDs/Journals List */}
              <div className="p-5 bg-[#FAFAFA] rounded-xl border border-gray-100">
                <label className="text-[14px] text-gray-800 font-medium mb-3 block">Papers in High-Impact Journals / CPD Programmes (Last 12 Months)</label>
                <div className="space-y-3">
                  {lecturer.journals.map((journal: any) => (
                    <div key={journal.id} className="flex items-center gap-3">
                      <input type="text" placeholder="Title of Paper or CPD" value={journal.title} onChange={(e) => updateListReq(lecturer.id, journal.id, 'journals', 'title', e.target.value)} className="flex-1 border border-[#BDE0A6] rounded-md p-2.5 outline-none focus:ring-2 focus:ring-[#5D9C0E]/50 text-[13px] bg-white transition-all" />
                      {lecturer.journals.length > 1 && (
                        <button onClick={() => removeListReq(lecturer.id, journal.id, 'journals')} className="p-2.5 text-gray-400 hover:text-red-500 shrink-0"><Trash2 size={16} /></button>
                      )}
                    </div>
                  ))}
                </div>
                <button onClick={() => addListReq(lecturer.id, 'journals')} className="mt-3 text-[13px] text-[#78C822] font-medium hover:underline">+ Add another entry</button>
              </div>
